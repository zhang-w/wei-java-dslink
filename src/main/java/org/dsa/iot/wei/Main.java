...
public class Main extends DSLinkHandler {
  ...
  public void preInit() {
	...
	public void onResponderInitialized(DSLink link) {
		...
		initEventNode(node);
		initActionNode(node);
		LOGGER.info("Simulator {} initialized.", Main.NAME);
	}
	...
	private static void initEventNode(Node node) {
		...
		Objects.getDaemonThreadPool().scheduleWithFixedDelay(new Runnable() {
			...
			public void run() {
				...
				JsonObject jsObj = new JsonObject().put("voltage", randomVoltate);
				Value val = new Value(jsObj.toString());
				val.setSerializable(false);
				child.setValue(val);
				LOGGER.info("Generated new event on {}: {}", child.getPath(), val);
			}
