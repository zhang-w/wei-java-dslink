package com.ibm.iot.analytics.edge.simulators;
...
public class SimGatewayDevice0 {
	...
	public static void init(DSLink link) {
		...
		initEventNode(node);
		initActionNode(node);
		LOGGER.info("Simulator {} initialized.", SimGatewayDevice0.NAME);
	}
	...
	private static void initEventNode(Node node) {
		...
		Objects.getDaemonThreadPool().scheduleWithFixedDelay(new Runnable() {
			@Override
			public void run() {
				double randomVoltate = RANDOM.nextDouble() * 3.3;
				JsonObject jsObj = new JsonObject().put("voltate", randomVoltate);
				Value val = new Value(jsObj.toString());
				val.setSerializable(false);
				child.setValue(val);
				LOGGER.info("Generated new event: {}", val);
			}
